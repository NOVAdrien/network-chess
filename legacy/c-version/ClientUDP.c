...
void receive_game_state_from_server(int client_socket, struct sockaddr_in *server_addr);
...
// Fonction pour recevoir l'échiquier depuis le serveur
void receive_game_state_from_server(int client_socket, struct sockaddr_in *server_addr) {
    socklen_t addr_len = sizeof(*server_addr);
    int bytes_received = recvfrom(client_socket, &game_state, sizeof(GameState), 0, (struct sockaddr *)server_addr, &addr_len);
    if (bytes_received < 0) {
        perror("Erreur lors de la réception de l'échiquier\n");
        exit(EXIT_FAILURE);
    }
    printf("Plateau reçu du serveur\n");
}

// Fonction pour envoyer l'échiquier au serveur
void send_game_state_to_server(int client_socket, struct sockaddr_in *server_addr) {
    int bytes_sent = sendto(client_socket, &game_state, sizeof(GameState), 0, (struct sockaddr *)server_addr, sizeof(*server_addr));
    if (bytes_sent < 0) {
        perror("Erreur lors de l'envoi de l'échiquier au serveur\n");
        exit(EXIT_FAILURE);
    }
    printf("Plateau envoyé au serveur\n");
}
...
void start_client(const char *server_ip) {
    ...
    // Envoyer un message initial pour se connecter au serveur
    char connect_msg = 'C';  // Message de connexion
    if (sendto(client_socket, &connect_msg, sizeof(connect_msg), 0, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
        perror("Erreur lors de l'envoi du message initial\n");
        close(client_socket);
        exit(EXIT_FAILURE);
    }
    ...
    printf("En attente de son id\n");
    if (recvfrom(client_socket, &client_id, sizeof(client_id), 0, NULL, NULL) < 0) {
    ...
    receive_game_state_from_server(client_socket, &server_addr);

    // Inversion du board uniquement pour le client 2 qui joue les noirs
    if (client_id == 1) {
        reverse_board(game_state.board);
    }
    printf("Echiquier reçu\n");
    ...
    while (!game_state.quit) {
        ...
        if (game_state.is_white_turn != 1 - client_id) {
            ...
            if (game_state.game_over) {
                ...
                send_game_state_to_server(client_socket, &server_addr);
                receive_game_state_from_server(client_socket, &server_addr);
                if (client_id == 1){
                    reverse_board(game_state.board);
                }
        ...
        while (SDL_PollEvent(&e) != 0) {
            ...
            // Gestion du clique du joueur à qui c'ets le tour de jouer
            if (game_state.is_white_turn == 1 - client_id) {
                if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
