...
#include "network_chess/Constants.hpp"

using network_chess::constants::chess::BoardSize;
...
void init_board() {
    // Pièces blanches
    for (int i = 0; i < BoardSize; i++) game_state.board[6][i] = 1;  // Pions
    ...
    // Pièces noires
    for (int i = 0; i < BoardSize; i++) game_state.board[1][i] = 7;  // Pions
...
bool is_king_in_check() {
    ...
    for (int start_row = 0; start_row < BoardSize; start_row++) {
        for (int start_col = 0; start_col < BoardSize; start_col++) {
        ...
        for (int start_col = 0; start_col < BoardSize; start_col++) {
            if ((game_state.is_white_turn && game_state.board[start_row][start_col] >= 1 && game_state.board[start_row][start_col] <= 6) ||
               (!game_state.is_white_turn && game_state.board[start_row][start_col] >= 7 && game_state.board[start_row][start_col] <= 12)) {
                for (int end_row = 0; end_row < BoardSize; end_row++) {
                    for (int end_col = 0; end_col < BoardSize; end_col++) {
...
bool handle_move_from_client(int current_player_index) {
    ...
    if (selected_row < 0 || selected_row >= BoardSize ||
        selected_col < 0 || selected_col >= BoardSize ||
        target_row < 0 || target_row >= BoardSize ||
        target_col < 0 || target_col >= BoardSize) {
        selected_row = -1;
        selected_col = -1;
        return false;
    }
